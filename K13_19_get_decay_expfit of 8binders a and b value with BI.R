# Energy-Distance Decay Analysis with Exponential Fitting (Including Binding Interface)
...
# ===============================
# Function: Calculate Exponential Fit Parameters for BOTH raw and median (with interface inclusion)
# ===============================
calculate_exp_fit_parameters_both_with_interface <- function(input, assay_sele, anno_file, interface_cutoff = 5) {
  ...
  for (dist_assay in distance_assays) {
    ...
    # --- 使用所有数据（包括结合界面） ---
    # 不排除结合界面残基
    # main_df <- main_df[main_df$x > interface_cutoff, ]  # 这行代码已被移除
    ...
    overall_results$interface_cutoff <- interface_cutoff
    ...
    if (nrow(sheet_df) >= 5) {
      ...
      sheet_results$interface_cutoff <- interface_cutoff
    ...
    if (nrow(helix_df) >= 5) {
      ...
      helix_results$interface_cutoff <- interface_cutoff
    ...
    if (nrow(other_df) >= 5) {
      ...
      other_results$interface_cutoff <- interface_cutoff
...
# ===============================
# Main Analysis for All Assays (with interface inclusion) - BOTH raw and median
# ===============================
analyze_all_assays_both_with_interface <- function(input_template, anno_file, output_file = NULL, interface_cutoff = 5) {
  ...
  for (assay in assays) {
    ...
    result <- calculate_exp_fit_parameters_both_with_interface(
      input = input_file,
      assay_sele = assay,
      anno_file = anno_file,
...
# ===============================
# CODE 1: Overall Structure Only (One Plot) - Including Interface - BOTH raw and median with BI1/BI2 colors
# ===============================

# Function to create overall structure plots (including interface) with BI1/BI2 color scheme
create_overall_plot_both_with_interface <- function(results_data) {
...
# ===============================
# CODE 2: Secondary Structure Regions (One Plot) - Including Interface - BOTH raw and median with BI1/BI2 colors
# ===============================

# Function to create secondary structure plots (including interface) with BI1/BI2 color scheme
create_secondary_structure_plot_both_with_interface <- function(results_data) {
...
# ===============================
# Summary Statistics Function for BOTH data types with interface
# ===============================
print_summary_statistics_both_with_interface <- function(results_data) {
  cat("=== Summary Statistics (Diagonal Only, Both Data Types, Including Interface) ===\n")
...
# ===============================
# Execute Analysis - Including Binding Interface - BOTH raw and median with BI1/BI2 colors
# ===============================

# Run analysis including binding interface (≤5Å) with BOTH data types
results_both_with_interface <- analyze_all_assays_both_with_interface(
  ...
  output_file = "C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/20251108_revise_figures/f5/20251119/exponential_fit_results_both_diagonal_with_interface.csv",
  interface_cutoff = 5
)

# CODE 1: Create overall structure plot (including interface) with BI1/BI2 color scheme
overall_plot_both_with_interface <- create_overall_plot_both_with_interface(results_both_with_interface)

# Save overall plot using ggsave (outside the function)
if (!is.null(overall_plot_both_with_interface)) {
  ggsave(
    filename = "C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/20251108_revise_figures/f5/20251119/overall_structure_both_BI_colors_with_interface.pdf",
    plot = overall_plot_both_with_interface,
    width = 16,
    height = 6,
    device = cairo_pdf
  )
  cat("Overall structure plot (BI1/BI2 color scheme, including interface) saved.\n")
}

# CODE 2: Create secondary structure plot (including interface) with BI1/BI2 color scheme
secondary_plot_both_with_interface <- create_secondary_structure_plot_both_with_interface(results_both_with_interface)

# Save secondary structure plot using ggsave (outside the function)
if (!is.null(secondary_plot_both_with_interface)) {
  ggsave(
    filename = "C:/Users/36146/OneDrive - USTC/Manuscripts/K13_K19/figures/20251108_revise_figures/f5/20251119/secondary_structure_both_BI_colors_with_interface.pdf",
    plot = secondary_plot_both_with_interface,
    width = 16,
    height = 10,
    device = cairo_pdf
  )
  cat("Secondary structure plot (BI1/BI2 color scheme, including interface) saved.\n")
}

# Print summary statistics for both data types with interface
print_summary_statistics_both_with_interface(results_both_with_interface)

cat("=== Analysis Complete ===\n")
cat("1. Results for both raw and median data (including binding interface ≤5Å) saved\n")
...
cat("   - BI2 Median: Light Red (#FFB0A5)\n")